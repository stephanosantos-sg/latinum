...
let _snapUnsub = null, _applyingCloud = false, _lastPushJSON = null;
const cloudAvailable = () => typeof firebase !== "undefined";

/* MERGE sem perda: nunca sobrescreve progresso — funde os dois lados.
   Progresso só cresce; escolhas (rate/idioma) seguem o save mais recente. */
function mergeState(a, b) {
  a = a || {}; b = b || {};
  const newer = (a.savedAt || 0) >= (b.savedAt || 0) ? a : b;
  const older = newer === a ? b : a;
  const m = Object.assign({}, DEFAULT_STATE, older, newer); // newer vence nos escalares/prefs
  m.xp = Math.max(a.xp || 0, b.xp || 0);
  m.streak = Math.max(a.streak || 0, b.streak || 0);
  m.lastDay = (a.lastDay || "") > (b.lastDay || "") ? a.lastDay : b.lastDay;
  // uniões (progresso binário): true de qualquer lado permanece
  ["lessons", "readFab", "words"].forEach(k => { m[k] = Object.assign({}, a[k], b[k]); });
  // SRS/frases: por chave, mantém o item MAIS avançado (maior intervalo)
  ["srs", "sents"].forEach(k => {
    m[k] = Object.assign({}, a[k]);
    const bb = b[k] || {};
    for (const key in bb) {
      const cur = m[k][key];
      if (!cur || (bb[key].interval || 0) > (cur.interval || 0) || (bb[key].due || "") > (cur.due || "")) m[k][key] = bb[key];
    }
  });
  // gramática (Declinā!): soma acertos e erros dos dois lados
  m.gram = Object.assign({}, a.gram);
  const bg = b.gram || {};
  for (const key in bg) {
    const cur = m.gram[key];
    m.gram[key] = cur ? { ok: (cur.ok || 0) + (bg[key].ok || 0), bad: (cur.bad || 0) + (bg[key].bad || 0) } : bg[key];
  }
  // meta do dia: a do save mais recente
  m.day = newer.day || older.day;
  m.savedAt = Math.max(a.savedAt || 0, b.savedAt || 0);
  return m;
}
...
function initCloud() {
  ...
  firebase.auth().onAuthStateChanged(user => {
    _fbUser = user;
    if (user) { _syncStatus = "on"; startCloudSync(); }
    else { _syncStatus = "off"; if (_snapUnsub) { _snapUnsub(); _snapUnsub = null; } }
    renderSyncChip();
...
/* liga o sync automático em tempo real: escuta o doc e funde qualquer mudança */
function startCloudSync() {
  if (!_fbUser || !_fbDb) return;
  if (_snapUnsub) _snapUnsub();
  _snapUnsub = _fbDb.collection("users").doc(_fbUser.uid)
    .onSnapshot({ includeMetadataChanges: false }, snap => {
      const cloud = snap.exists ? snap.data().latinum : null;
      if (!cloud) { cloudPush(); return; } // nuvem vazia: sobe o que temos
      const before = JSON.stringify(S);
      const merged = mergeState(S, cloud);
      const mergedJSON = JSON.stringify(merged);
      if (mergedJSON !== before) {
        // a nuvem trouxe algo novo → aplica localmente (sem re-disparar push em loop)
        _applyingCloud = true;
        S = merged;
        localStorage.setItem("latinum", JSON.stringify(S));
        renderStats();
        const v = App._view || "home";
        if (v === "profile") renderProfile(); else if (v === "review") renderReview();
        _applyingCloud = false;
        if (!L && !window._ab) toast("☁️ Progresso sincronizado (" + S.xp + " XP)");
      }
      // se o merge tem algo que a nuvem ainda não tem, sobe
      if (mergedJSON !== JSON.stringify(cloud)) cloudPush(true);
    }, err => {
      console.error("cloudSync:", err); _syncStatus = "error"; renderSyncChip();
    });
}
async function cloudPush(force) {
  ...
  try {
    await _fbDb.collection("users").doc(_fbUser.uid).set({ latinum: JSON.parse(json) }, { merge: true });
    _lastPushJSON = json;
    _syncStatus = "on";
    const el = document.querySelector("#sync-status");
    if (el) el.textContent = "☁️ Sincronizado agora há pouco";
  } catch (e) { console.error("cloudPush:", e); _syncStatus = "error"; renderSyncChip(); }
}
/* botão manual "puxar": força uma releitura e merge imediatos */
async function cloudPull() {
  if (!_fbUser || !_fbDb) { toast("Entre com o Google primeiro"); return; }
  try {
    const doc = await _fbDb.collection("users").doc(_fbUser.uid).get();
    const cloud = doc.exists ? doc.data().latinum : null;
    S = mergeState(S, cloud);
    localStorage.setItem("latinum", JSON.stringify(S));
    renderStats(); cloudPush(true);
    toast("☁️ Progresso sincronizado (" + S.xp + " XP)");
    App.go("profile");
  } catch (e) { console.error("cloudPull:", e); toast("❌ Erro ao sincronizar: " + (e.code || e.message)); }
}
function scheduleCloudPush() {
  if (!_fbUser || _applyingCloud) return;
...
/* ============================================================
   ...
   ============================================================ */
/* barra de áudio: agora flutua fixa no rodapé (não empurra o texto) */
function abControls() { return ""; } // legado: os controles agora vivem em #ab-bar
function abBarHTML() {
  return `<button class="ab-btn ab-play" id="ab-play" onclick="abToggle()" title="ouvir / pausar">▶️</button>
    <button class="ab-btn" onclick="abStep(-1)" title="frase anterior">⏮</button>
    <button class="ab-btn" onclick="abStep(1)" title="próxima frase">⏭</button>
    <button class="ab-btn" onclick="abRepeat()" title="repete a frase">🔁</button>
    <button class="ab-btn" onclick="changeRate(-1)" title="mais devagar">🐢</button>
    <span class="ab-rate tts-rate">${rateLabel()}</span>
    <button class="ab-btn" onclick="changeRate(1)" title="mais rápido">🐇</button>
    <button class="ab-btn ${S.hideMacrons ? "toggled" : ""}" onclick="toggleMacrons()" title="esconde/mostra macrons">ā/a</button>`;
}
function showAbBar() {
  const bar = document.getElementById("ab-bar");
  if (!bar) return;
  bar.innerHTML = abBarHTML();
  bar.hidden = false;
...
function hideAbBar() {
  ...
  document.body.classList.remove("ab-active");
}
function abInit() {
...
function abToggle() {
  ...
  if (ab.playing) {
    ab.playing = false;
    speechSynthesis.cancel();
    const b = $("#ab-play"); if (b) b.textContent = "▶️";
  } else {
    ab.playing = true;
    const b = $("#ab-play"); if (b) b.textContent = "⏸";
